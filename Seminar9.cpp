...
public:
	...
	Librarie& operator=(const Librarie& l) {
		if (&l != this)
		{
			Magazin::operator=(l); // apelare operator = din Magazin
			if (this->nume)
				delete[] this->nume;
			this->nume = new char[strlen(l.nume) + 1];
			strcpy_s(this->nume, strlen(l.nume) + 1, l.nume);
	...
	friend ostream& operator<<(ostream& os, const Librarie& l) {
		os << (Magazin)l<<endl;                                           // cast la Magazin, ca sa afiseze si elementele din Magazin
		os << "Numele librariei: " << l.nume<<endl;
		os << "Nr de carti: " << l.nrCarti<<endl;
		return os;
	}

	operator int() {
		return this->nrCarti;   // op de cast la int pt Librarie, care va afisa nr de carti
	}
};
...
int main() {
	...
	Librarie l3("Graphotek", 60, 3, "Galati", salarii, 200, 2010);
	Librarie l4(l3);  //apelare constructor de copiere;
	Librarie l5 = l3; // apelare constructor de copiere
	Librarie l6;
	l6 = l3; // apelare operator =
	...
	cout <<endl << "Numarul mediu de carti per angajat: " << l3.getNrMediuCartiPerAngajat();
