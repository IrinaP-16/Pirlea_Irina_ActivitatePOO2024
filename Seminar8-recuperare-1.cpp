...
public:
	...
	ComplexComercial(Magazin mag, Cofetarie* cof, int nrMasina, Masina* masini)
	{
		this->magazin = mag;
		this->cofetarie = new Cofetarie(*cof);
		this->nrMasiniParcate = nrMasina;
		this->masiniParcate = new Masina[nrMasina];
		for (int i = 0; i < nrMasina; i++) {
	...
	friend ostream& operator<<(ostream& o, const ComplexComercial& cc) {
		o << "Magazinul din complexul comercial: " << cc.magazin << endl;
		o << "Cofetaria din complex este:" << endl;
		o << cc.cofetarie << endl;
		o << "Avem" << cc.nrMasiniParcate << "masini";
		for (int i = 0; i < cc.nrMasiniParcate; i++)
		{
