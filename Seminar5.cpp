...
public:
	...
	explicit operator int() { //operator de cast
		return this->nrAngajati;
	}

	float operator[](int index) {
		if (index >= 0 && index < this->nrAngajati) {  // verificare daca e diferit de 0 ca la get
			return this->salarii[index];
		}
		else
		{
	...
	friend ostream& operator<<(ostream& os, const Magazin& m)   //ostream& pt apel in cascada  -- functie in interiorul clasei(avem friend)
	{
		os << "Orasul: " << m.oras << endl;
		os << "Nr Angajati: " << m.nrAngajati << endl;
		if (m.nrAngajati > 0) {
			for (int i = 0; i < m.nrAngajati; i++)
			{
				os << m.salarii[i] << " ";
		...
		os << endl;
		os << "Suprafata: " << m.suprafata << endl;
		os << "An deschidere: " << m.anDeschidere << endl;
		os << "Impozit: " << m.impozitM2 << endl;
