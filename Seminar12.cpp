...
public:
	...
	Vector(int lungime, Model** elemente) {  // constructor cu parametri
		this->lungime = lungime;
		this->elemente = new Model[lungime];
		for (int i = 0; i < lungime; i++)
			this->elemente[i] = new Model(*elemente[i]);  // dereferentierea adresei , deep copy
	}
	...
	Vector operator=(const Vector& v) { // operatorul =
		if (&v != this) {
			this->lungime = v.lungime;
	...
	void AdaugaElement(Model *element) {
		Model** aux = new Model*[this->lungime+1];// elementele din vectorul de pointeri** sunt de tipul pointer*
...
int main() {
	...
	Magazin* m=new Magazin("Galati",3,100,2024);// cand alocam cu new se aloca memorie pe heap, altfel se aloca memorie in stack
